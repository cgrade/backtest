...
const TFS = ["1H", "45M", "30M", "15M"];
const PAIRS = ["XAUUSD","EURUSD","GBPUSD","GBPJPY","USDJPY","EURJPY","NAS100","US30","BTCUSD","Other"];
const STORE_KEY = "slp_backtest_v1";

const fresh = () => ({
  id: `${Date.now()}-${Math.random().toString(36).slice(2,5)}`,
  date: new Date().toISOString().split("T")[0],
  pair: "XAUUSD", customPair: "",
  tradeType: null,
  bias: null,
  klType: null, klReacted: null,
  mssFormed: null, mssTF: null, mssDir: null,
  doubleBOS: null, doubleBOSTF: null, doubleBOSDir: null,
  induced: null,
  poiType: null, poiTF: null, poiReacted: null,
  tradeTaken: null, result: null, rr: "",
  notes: ""
});

const pct = (n: number, d: number) => d ? Math.round((n / d) * 100) : 0;
const getType = (e: any) => e.tradeType || "reversal";
...
interface Entry {
  ...
  bias: string | null;
  klType?: string | null;
  klReacted?: string | null;
  mssFormed?: string | null;
  mssTF?: string | null;
  mssDir?: string | null;
  doubleBOS?: string | null;
  doubleBOSTF?: string | null;
  doubleBOSDir?: string | null;
  induced?: string | null;
  poiType?: string | null;
  poiTF?: string | null;
  poiReacted?: string | null;
  tradeTaken?: string | null;
  result?: string | null;
  rr: string;
  notes: string;
}
...
export default function SLPTracker() {
  ...
  const handleTypeChange = (v: "reversal" | "continuation" | null) => {
  setForm(p => ({
    ...p as Entry,
    tradeType: v,
    klType: null, klReacted: null, mssFormed: null, mssTF: null, mssDir: null,
    doubleBOS: null, doubleBOSTF: null, doubleBOSDir: null,
    induced: null, poiType: null, poiTF: null, poiReacted: null,
    tradeTaken: null, result: null, rr: "",
  }));
};
  ...
  const revFunnel = [
    { label: "Reversal setups", n: REV.length, d: null },
  ...
  function Funnel({title,steps,warm}: {title: string, steps: any[], warm: boolean}) {
    const colors=warm
            ...
            <div key={i} style={{marginBottom:i<steps.length-1?"12px":"0"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:"3px"}}>
                ...
                <span style={{fontSize:"13px",fontWeight:"500",color:tcs[i]||"var(--color-text-primary)"}}>
                  {s.n}{s.d&&<span style={{fontSize:"11px",color:"var(--color-text-secondary)",fontWeight:"400"}}> ({p}% of prev)</span>}
                </span>
              </div>
  ...
  function TFBar({items,total,color}: {items: any[], total: number, color: string}) {
                    ...
                    {rMssF.length>0&&(
                      <div style={{background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"14px 16px",marginBottom:"12px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
                          ...
                          <div style={{fontSize:"11px",color:"var(--color-text-secondary)"}}>{rMssF.filter((e: Entry)=>e.mssDir==="Bullish").length}▲ · {rMssF.filter((e: Entry)=>e.mssDir==="Bearish").length}▼</div>
                        </div>
                    ...
                    {cBOS.length>0&&(
                      <div style={{background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"14px 16px",marginBottom:"12px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
                          ...
                          <div style={{fontSize:"11px",color:"var(--color-text-secondary)"}}>{cBOS.filter((e: Entry)=>e.doubleBOSDir==="Bullish").length}▲ · {cBOS.filter((e: Entry)=>e.doubleBOSDir==="Bearish").length}▼</div>
                        </div>
